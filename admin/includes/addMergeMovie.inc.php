...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['upload_product_submit'])){
    ...
    $grand_order_unit = htmlspecialchars($_POST['grand_order_unit']) ??'';

    //extra data
    $username = $_POST['username'];
    $acctNo = $_POST['account_number'];
    ...
    if(!empty($_FILES['product_img']['name'])){
        ...
        if($productImg_fileError === 0){
            ...
            if($productImg_fileSize < 10000000){
                ...
                header("location: ../accounts/addMergeMovie.php?msg=ImgSizeTooLarge");
            }



        }else{
            header("location: ../accounts/addMergeMovie.php?msg=ImgErrorOccured");
        }



    }else{
        header("location: ../accounts/addMergeMovie.php?msg=notSupportedImg");
    }
        ...
        header("location: ../accounts/addMergeMovie.php?msg=uploadImg");
    }
    ...
    if(empty($product_title) || empty($product_amount) || empty($commission) || empty($level) ){
        header("location: ../accounts/addMergeMovie.php?msg=fillAllFields");
    }
    else{

    //Run insertion into database
    $product_query = "INSERT INTO merge_product (username, acctNo, product_title, product_amount, order_number, commission, level, product_img, grand_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);";
    $stmtProduct = mysqli_prepare($conn, $product_query);

    mysqli_stmt_bind_param($stmtProduct, "sssssssss", $username, $acctNo, $product_title, $product_amount, $orderNumber, $commission, $level, $productImg_destinationFile, $grand_order_unit);
    ...
    header("location: ../accounts/addMergeMovie.php?msg=success");
    ...
    if(empty($product_title) || empty($product_amount) || empty($commission) || empty($level) ){
        ...
        $sql = "UPDATE merge_product SET product_title = ?, product_amount = ?,  product_img = ?, grand_order = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
    
        mysqli_stmt_bind_param($stmt, "sssss", $product_title, $product_amount,  $productImg_destinationFile, $grand_order_unit, $id);
        mysqli_stmt_execute($stmt);
    
        header("location: ../accounts/addMergeMovie.php?msg=success&page=update&id=$id");
