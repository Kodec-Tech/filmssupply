...
$(document).ready(function () {
    ...
    $('#AdharNo').blur(function () {
        ...
        if (AdharNo == "" || AdharNo.length > 15 || AdharNo.length < 8) {
            $('#AdharError').text('Invalid National ID Number');
    ...
    $('#PanNo').blur(function () {
        ...
        if (PanNo == "") {
            ...
            if (!regex.test(PanNo)) {
                ...
                $('#PanError').text('Invalid Address Format');
                $("#nextBtn").attr('disabled', true);
                PanError = 1;
            }
            else {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response != "0") {
                            $("#PanError").text("Address Already Exist");
                            $("#nextBtn").attr('disabled', true);
                            PanError = 1;
                        }
                        else {
    ...
    $("#pincode").blur(function () {
        ...
        if (pincode == "") {
            ...
            $("#PincodeError").text("Please Enter Your Area Zipcode");
            $("#nextBtn").attr('disabled', true);
            pincodeError = 1;
        }
        else {

            let match = /^[A-Za-z0-9\s]+$/;

            if (!match.test(pincode)) {

                $('#PincodeError').text('Invalid Zipcode');
                $("#nextBtn").attr('disabled', true);
                PincodeError = 1;
            }
            else {
    ...
    $("#FirstName").blur(function () {
        ...
        if (Fname == "") {
            ...
            if (!Expression.test(Fname)) {
                $("#FnameError").text("Name doesn't contain Numbers or Symbols");
                $("#nextBtn").attr('disabled', true);
                FnameError = 1;
            }
            else {
    ...
    $("#Lname").blur(function () {
        ...
        if (Lname == "") {
            ...
            if (!Expression1.test(Lname)) {
                $("#LnameError").text("Last Name doesn't contain Numbers or Symbols");
                $("#nextBtn").attr('disabled', true);
                LnameError = 1;
            }
            else {
    ...
    $("#FAname").blur(function () {
        ...
        if (FAname == "") {
            $("#FAnameError").text("Please Enter your Next Of Kin's Full Name");
            $("#nextBtn").attr('disabled', true);
            FAnameError = 1;
        }
        else {
    ...
    $("#MAname").blur(function () {
        ...
        let Expression3 = /^[0-9-+()]*$/;
        if (MAname == "") {
            $("#MAnameError").text("Please Enter your Next Of Kin's Number");
            $("#nextBtn").attr('disabled', true);
            MAnameError = 1;
        }
        else {
            ...
            if (!Expression3.test(MAname)) {
                $("#MAnameError").text("Numbers and Special Characters (+) Only ");
                $("#nextBtn").attr('disabled', true);
                MAnameError = 1;
            }
            else {
